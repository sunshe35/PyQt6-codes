...
class QTreeWidgetDemo(QMainWindow):
    ...
    def initItem(self):
        ...
        # 设置根节点的背景颜色
        root.setBackground(0, QBrush(Qt.GlobalColor.blue))
        root.setBackground(1, QBrush(Qt.GlobalColor.yellow))
        root.setBackground(2, QBrush(Qt.GlobalColor.red))
        ...
        for subject in ['语文', '数学', '外语', '综合']:
            ...
            for name in ['张三', '李四', '王五', '赵六']:
                ...
                child2.setFlags(Qt.ItemFlag.ItemIsSelectable | Qt.ItemFlag.ItemIsEditable | Qt.ItemFlag.ItemIsEnabled | Qt.ItemFlag.ItemIsUserCheckable)
                child2.setText(1, name)
                score = random.random() * 40 + 60
                child2.setText(2, str(score)[:5])
                if score >= 90:
                    child2.setBackground(2, QBrush(Qt.GlobalColor.red))
                elif 80 <= score < 90:
                    child2.setBackground(2, QBrush(Qt.GlobalColor.darkYellow))
