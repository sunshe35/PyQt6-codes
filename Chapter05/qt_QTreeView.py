...
class QTreeViewDemo(QMainWindow):
    ...
    def initItem(self):
        ...
        # 设置根节点的背景颜色
        root.setBackground(QBrush(Qt.GlobalColor.blue))
        rootList[1].setBackground(QBrush(Qt.GlobalColor.yellow))
        rootList[2].setBackground(QBrush(Qt.GlobalColor.red))
        ...
        for subject in ['语文', '数学', '外语', '综合']:
            ...
            for name in ['张三', '李四', '王五', '赵六']:
                ...
                itemName.setFlags(Qt.ItemFlag.ItemIsSelectable | Qt.ItemFlag.ItemIsEditable | Qt.ItemFlag.ItemIsEnabled | Qt.ItemFlag.ItemIsUserCheckable)
                score = random.random() * 40 + 60
                itemScore = QStandardItem(str(score)[:5])
                if score >= 90:
                    itemScore.setBackground(QBrush(Qt.GlobalColor.red))
                elif 80 <= score < 90:
                    itemScore.setBackground(QBrush(Qt.GlobalColor.darkYellow))
