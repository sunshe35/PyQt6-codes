...
class QTableViewDemo(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        cusHeaderItem.setTextAlignment(Qt.AlignmentFlag.AlignVCenter)
        ...
        self.tableView.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)  ######允许右键产生子菜单
    ...
    def initItem(self):
        ...
        for row in range(self.model.rowCount()):
            for column in range(self.model.columnCount()):
                ...
                item.setData(QColor(155, 14, 0), role=Qt.ItemDataRole.ForegroundRole)
                item.setData(value + '-toolTip', role=Qt.ItemDataRole.ToolTipRole)
                item.setData(value + '-statusTip', role=Qt.ItemDataRole.StatusTipRole)
                item.setData(QIcon("images/open.png"), role=Qt.ItemDataRole.DecorationRole)
                self.model.setItem(row, column, item)

        # flag+check
        item = QStandardItem('flag+check1')
        item.setFlags(Qt.ItemFlag.ItemIsSelectable | Qt.ItemFlag.ItemIsEditable | Qt.ItemFlag.ItemIsEnabled | Qt.ItemFlag.ItemIsUserCheckable)
        ...
        self.actionHelp = QWhatsThis.createAction(self)
        self.actionHelp.setText('显示whatsThis-help')
        self.actionHelp.setShortcut(QKeySequence(Qt.Modifier.CTRL | Qt.Key.Key_H))
        ...
        tool_button.clicked.connect(lambda: QWhatsThis.enterWhatsThisMode())

        self.model.setData(self.model.index(4, 0), QColor(215, 214, 220), role=Qt.ItemDataRole.BackgroundRole)
    ...
    def generateMenu(self):
        ...
        menu.addAction(QIcon("images/close.png"), '删除列',
                       QKeySequence(Qt.Modifier.CTRL | Qt.Modifier.SHIFT | Qt.Key.Key_D), lambda: self.onDelete('column'))
        menu.addSeparator()
