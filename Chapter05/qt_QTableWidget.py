...
class QTableWidgetDemo(QMainWindow):
    ...
    def initItem(self):
        ...
        # self.actionHelp.setShortcuts(QKeySequence(Qt.Modifier.CTRL | Qt.Key.Key_H))
        # self.actionHelp.setShortcuts(QKeySequence(Qt.Modifier.CTRL | Qt.Key.Key_H))
    ...
    def onItemClicked(self, item: QTableWidgetItem):
        ...
        # if row == 0:
        #     _str1 = f'当前点击:"{item.text()}",上一个：None,下一个:"{self.tableWidget.item(row + 1).text()}"'
        # elif row == self.tableWidget.count() - 1:
        #     _str1 = f'当前点击:"{item.text()}",上一个："{self.tableWidget.item(row - 1).text()}",下一个:None'
        # else:
        #     _str1 = f'当前点击:"{item.text()}",上一个："{self.tableWidget.item(row - 1).text()}",下一个:"{self.tableWidget.item(row + 1).text()}"'
