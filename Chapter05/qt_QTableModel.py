...
class StudentTableModel(QAbstractTableModel):
    ...
    def data(self, index, role=Qt.ItemDataRole.DisplayRole):
        ...
        elif role == Qt.ItemDataRole.ForegroundRole and column == SCORE:
            if student.score < 80:
    ...
    def setData(self, index, value, role=Qt.ItemDataRole.EditRole):
        if index.isValid() and 0 <= index.row() < len(self.students) and role==Qt.ItemDataRole.EditRole:
            student = self.students[index.row()]
            column = index.column()
            if column == SUBJECT:
                ...
                try:
                    ...
                    print('输入错误，请输入数字')
            # self.signalEmit.emit(SIGNAL("dataChanged(QModelIndex,QModelIndex)"), index, index)
