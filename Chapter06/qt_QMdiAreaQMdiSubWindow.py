...
class MdiAreaDemo(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.nextAct.setShortcuts(QKeySequence.StandardKey.New)
        self.nextAct.triggered.connect(self.mdi.activateNextSubWindow)
        file.addAction(self.nextAct)
        self.preAct = QAction('Pre')
        self.preAct.setShortcut(QKeySequence(Qt.Modifier.CTRL | Qt.Key.Key_P))
        ...
        mdiSub3.setOption(mdiSub3.SubWindowOption.RubberBandMove, on=True)
        mdiSub3.setOption(mdiSub3.SubWindowOption.RubberBandResize, on=True)
    ...
    def orderAction(self, q):
        if q.text() == 'create':
            self.mdi.setActivationOrder(self.mdi.WindowOrder.CreationOrder)
        elif q.text() == 'stack':
            self.mdi.setActivationOrder(self.mdi.WindowOrder.StackingOrder)
        elif q.text() == 'activateHistory':
            self.mdi.setActivationOrder(self.mdi.WindowOrder.ActivationHistoryOrder)
        self.showInfo()
    ...
    def viewAction(self, q):
        if q.text() == 'subWindow':
            ...
            self.mdi.setViewMode(self.mdi.ViewMode.TabbedView)
