...
class DockWidgetDemo(QMainWindow):
    ...
    def createDock1Window(self):
        ...
        self.addDockWidget(Qt.DockWidgetArea.RightDockWidgetArea, dockWidget1)

    def createDock2Window(self):
        dockWidget2 = self.createDockWidget(title="Dockable2-限制停靠区域:右|下", n=2)
        dockWidget2.setAllowedAreas(Qt.DockWidgetArea.RightDockWidgetArea | Qt.DockWidgetArea.BottomDockWidgetArea)
        self.addDockWidget(Qt.DockWidgetArea.RightDockWidgetArea, dockWidget2)

    def createDock3Window(self):
        dockWidget3 = self.createDockWidget(title="Dockable3-浮动", n=3)
        dockWidget3.setFloating(True)
        self.addDockWidget(Qt.DockWidgetArea.RightDockWidgetArea, dockWidget3)

    def createDock4Window(self):
        dockWidget4 = self.createDockWidget(title="Dockable4-关闭/移动/左侧栏", n=4)
        self.addDockWidget(Qt.DockWidgetArea.LeftDockWidgetArea, dockWidget4)
        dockWidget4.setFeatures(QDockWidget.DockWidgetFeature.DockWidgetClosable | QDockWidget.DockWidgetFeature.DockWidgetMovable|QDockWidget.DockWidgetFeature.DockWidgetVerticalTitleBar)

    def createDock5Window(self):
        dockWidget5 = self.createDockWidget(title="Dockable5-修改自定义标题", n=5)
        self.addDockWidget(Qt.DockWidgetArea.LeftDockWidgetArea, dockWidget5)
