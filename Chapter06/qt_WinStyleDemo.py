...
class WinStyleDemo(QMainWindow):
    def __init__(self, parent=None):
        ...
        # 选择当前界面风格
        index = self.styleComboBox.findText(QApplication.style().objectName(), Qt.MatchFlag.MatchFixedString)
        # 设置当前界面风格
    ...
    #     if button.isChecked() and button.text() == 'paintEvent':
    #         self._paintEvent(event)
    #     super(WinStyleDemo,self).paintEvent(event)

    def _paintEvent(self, event):
        painter = QPainter(self)
        painter.setBrush(Qt.GlobalColor.green)
        # 设置背景颜色，左半屏幕
        painter.drawRect(0, 0, int(self.width() / 2), int(self.height()))
        # 设置背景图片，右半屏幕
        pixmap = QPixmap("./images/screen1.jpg")
        painter.drawPixmap(int(self.width() / 2), 0, int(self.width() / 2), int(self.height()), pixmap)
    ...
    def winFlagChanged(self, a: int):
        ...
        self.setWindowFlag(getattr(Qt.WindowType, flag))
        self.show()
    ...
    def updateWinFlag(self):
        ...
        _text += flagText
        flag = getattr(Qt.WindowType, flagText)
        for checkButton in self.checkBoxList:
            if checkButton.isChecked():
                flag |= getattr(Qt.WindowType, checkButton.text())
    ...
    def updateBackColor(self, button: QRadioButton):
        ...
        if text == 'setStyleSheet':
            ...
            # QPalette设置
            palette = QPalette()
            palette.setColor(QPalette.ColorRole.ButtonText, Qt.GlobalColor.darkCyan)
