...
class DialogButtonBox(QWidget):
    ...
    def show_dialog(self, buttonBox):
        ...
        self.dialog.setWindowTitle("Dialog + QDialogButtonBox demo")
        layout = QVBoxLayout()
        layout.addWidget(QLabel('QDialogButtonBox嵌入到对话框中实例'))
        layout.addWidget(buttonBox)
        self.dialog.setLayout(layout)
        self.dialog.move(self.geometry().x(), self.geometry().y() + 180)
        # 绑定相应信号与槽，用于退出对话框
        buttonBox.accepted.connect(self.dialog.accept)
        buttonBox.rejected.connect(self.dialog.reject)
        buttonBox.setOrientation(Qt.Orientation.Vertical)  # 垂直排列
        self.dialog.exec()

    def create_buttonBox(self):
        buttonBox = QDialogButtonBox()
        buttonBox.setStandardButtons(
            QDialogButtonBox.StandardButton.Cancel | QDialogButtonBox.StandardButton.Ok | QDialogButtonBox.StandardButton.Reset | QDialogButtonBox.StandardButton.Help | QDialogButtonBox.StandardButton.Yes | QDialogButtonBox.StandardButton.No | QDialogButtonBox.StandardButton.Apply)
        # 自定义按钮
