...
class InputdialogDemo(QWidget):
    ...
    def getInt(self):
        num, ok = QInputDialog.getInt(self, "integer input dualog", "输入数字", 10, min=-10, max=120, step=10)
        if ok:
            self.le3.setText(str(num))

    def getDouble(self):
        num, ok = QInputDialog.getDouble(self, "double input dualog", "输入数字", 5, min=-1.00, max=20.00,
                                         decimals=2, step=0.1)
        if ok:
